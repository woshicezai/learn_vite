...
const processedColumns = computed(() => {
  ...
  // 冻结列总宽度
  const totalFrozenWidth = frozenLeft;

  // 计算非冻结列的start位置，从冻结列的总宽度开始计算
  let nonFrozenStart = totalFrozenWidth;
  const processedNonFrozen = nonFrozen.map((col) => {
    ...
    const processed = {
      ...
      id: col.prop || `col-${col.originalIndex}`,
      width,
      start: nonFrozenStart, // 从冻结列总宽度开始
...
const visibleNonFrozenColumns = computed(() => {
  ...
  // 计算可见视口
  const viewportStart = scrollLeft.value + frozenWidth.value;
  const viewportEnd = viewportStart + props.width - frozenWidth.value;

  // 找到第一个可见列
  const startIndex = columns.findIndex((col) => col.end > viewportStart);
...
const scrollToColumn = (colIndex) => {
  ...
  // 滚动到目标列的开始位置（减去冻结列宽度得到实际滚动位置）
  bodyContainerRef.value.scrollLeft =
    targetInNonFrozen.start - frozenWidth.value;
};
